...
import { AlertTriangle, CheckCircle2, Download, FileUp, HandCoins, Upload, XCircle } from "lucide-react";
import type { PreviewRow } from "@/lib/validation/donation-import";
...
export function DonationImportWizard() {
  const router = useRouter();
  const t = useTranslations("donations.importWizard");
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/donations/import/preview", { method: "POST", body: formData });
  ...
  async function handleImport() {
    ...
    setStep("importing");
    try {
      const response = await fetch("/api/donations/import/commit", {
        method: "POST",
  ...
  return (
    <div className="space-y-4">
        ...
        <Card className="glass-card rounded-2xl border-dashed">
          <CardContent className="flex flex-col items-center gap-4 py-12 text-center">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              ...
              <Link
                href="/api/donations/import/template"
                prefetch={false}
      ...
      {step === "reviewing" && preview && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <CountChip label={t("countChips.total")} value={preview.totalRows} icon={HandCoins} />
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden md:table-cell">{t("columns.amount")}</TableHead>
              ...
              <TableBody>
                {visibleRows.map((row) => (
                  <TableRow key={row.rowNumber}>
                    ...
                    <TableCell>{row.data.donorName || "—"}</TableCell>
                    <TableCell className="hidden md:table-cell">
                      {row.data.amount !== null ? formatInr(row.data.amount) : "—"}
                    </TableCell>
                    <TableCell className="hidden lg:table-cell">{row.data.purpose || "—"}</TableCell>
                    <TableCell className="hidden lg:table-cell">
                      {row.data.paymentMethod ? tPaymentMethods(row.data.paymentMethod) : "—"}
                    </TableCell>
                    <TableCell className="hidden md:table-cell">{row.data.donatedAt || "—"}</TableCell>
                    <TableCell>
      ...
      {step === "upload" && (
        <Link href="/dashboard/donations" className="text-sm text-muted-foreground underline-offset-2 hover:underline">
          {t("backToDonations")}
        </Link>
      )}
    </div>
