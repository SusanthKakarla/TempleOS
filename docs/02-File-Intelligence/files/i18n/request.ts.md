...
Localization in the **Internationalization** area. It defines project behavior, structure, data, or configuration consumed by its dependents.
...
- Inputs: imports from `next-intl/server`, `@/lib/i18n/locale`.
- Outputs: side effects or static artifact.

## Dependencies

- Internal imports: `lib/i18n/locale.ts`
...
`Runtime/framework → i18n/request.ts → lib/i18n/locale.ts`
