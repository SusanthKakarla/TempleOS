...
| Field | Value |
|---|---|
| Full path | `lib/validation/temple-faqs.ts` |
| Layer | Domain |
| Category | Validation |
...
Public symbols: `createFaqSchema`, `updateFaqSchema`, `CreateFaqPayload`, `UpdateFaqPayload`.
...
- Outputs: exports `createFaqSchema`, `updateFaqSchema`, `CreateFaqPayload`, `UpdateFaqPayload`.
...
- Imported by: `app/api/temple-faqs/[id]/route.ts`, `app/api/temple-faqs/route.ts`, `lib/validation/temple-faqs.test.ts`
...
`Runtime/framework → lib/validation/temple-faqs.ts → output or side effect`
