...
| Field | Value |
|---|---|
| Full path | `lib/validation/temple-time.test.ts` |
| Layer | Testing |
| Category | Test |
...
- Inputs: imports from `vitest`, `./temple-time`.
...
`Runtime/framework → lib/validation/temple-time.test.ts → lib/validation/temple-time.ts`
