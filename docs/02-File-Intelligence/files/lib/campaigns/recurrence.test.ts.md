...
| Field | Value |
|---|---|
| Full path | `lib/campaigns/recurrence.test.ts` |
| Layer | Testing |
| Category | Test |
...
- Inputs: imports from `vitest`, `./recurrence`.
- Outputs: side effects or static artifact.

## Dependencies

- Internal imports: `lib/campaigns/recurrence.ts`
...
- Imports: `lib/campaigns/recurrence.ts`
...
`Runtime/framework → lib/campaigns/recurrence.test.ts → lib/campaigns/recurrence.ts`
