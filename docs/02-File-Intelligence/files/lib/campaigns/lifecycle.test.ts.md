...
| Field | Value |
|---|---|
| Full path | `lib/campaigns/lifecycle.test.ts` |
| Layer | Testing |
| Category | Test |
...
- Inputs: imports from `vitest`, `./lifecycle`.
- Outputs: side effects or static artifact.

## Dependencies

- Internal imports: `lib/campaigns/lifecycle.ts`
...
- Imports: `lib/campaigns/lifecycle.ts`
...
`Runtime/framework → lib/campaigns/lifecycle.test.ts → lib/campaigns/lifecycle.ts`
