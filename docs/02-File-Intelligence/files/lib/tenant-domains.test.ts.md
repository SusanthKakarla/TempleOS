...
| Field | Value |
|---|---|
| Full path | `lib/tenant-domains.test.ts` |
| Layer | Testing |
| Category | Test |
...
- Inputs: imports from `vitest`, `./tenant-domains`.
- Outputs: side effects or static artifact.

## Dependencies

- Internal imports: `lib/tenant-domains.ts`
...
`Runtime/framework → lib/tenant-domains.test.ts → lib/tenant-domains.ts`
