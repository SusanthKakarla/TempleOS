...
| Field | Value |
|---|---|
| Full path | `lib/auth/session-token.test.ts` |
| Layer | Testing |
| Category | Test |
...
- Inputs: imports from `vitest`, `./session-token`; environment: `SESSION_SECRET`.
...
- File size: 61 lines; 1 internal dependencies.
...
`Runtime/framework → lib/auth/session-token.test.ts → lib/auth/session-token.ts`
