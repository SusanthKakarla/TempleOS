# festival-greetings.ts

## Basic Information

| Field | Value |
|---|---|
| Full path | `lib/db/festival-greetings.ts` |
| Layer | Database |
| Category | Repository |
...
Public symbols: `enqueueFestivalGreeting`.
...
- Outputs: exports `enqueueFestivalGreeting`.
...
- Imported by: `app/api/notification-media/[id]/send-festival-greeting/route.ts`
...
`Runtime/framework → lib/db/festival-greetings.ts → lib/db/pool.ts / lib/db/notification-templates.ts / types/db.ts`
