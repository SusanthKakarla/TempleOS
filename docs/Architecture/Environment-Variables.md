...
| `TEMPLEOS_WEBSITE_DOMAIN` | Optional, defaults to `templos.in` | `lib/site/website-host.ts` | Nothing breaks loudly — public temple websites keep resolving on the code default. Set it per deployment so staging serves websites from its own domain and production never silently depends on a hardcoded fallback. Requires a wildcard `*.<domain>` DNS record pointing at this same deployment |
