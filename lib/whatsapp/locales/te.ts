import type { LocaleDictionary } from "./types";

export const te: LocaleDictionary = {
  menuGreetingFallback: "🙏 {temple} దేవాలయానికి స్వాగతం.",
  menuButtonLabel: "మెను",
  menuSectionTitle: "ఎంపికలు",
  menuRowEventsTitle: "కార్యక్రమాలు",
  menuRowEventsDescription: "రాబోయే దేవాలయ కార్యక్రమాలు",
  menuRowContactTitle: "సంప్రదించండి",
  menuRowContactDescription: "ఫోన్, చిరునామా, మ్యాప్",
  menuRowTimingsTitle: "సమయాలు",
  menuRowTimingsDescription: "నేటి తెరిచే సమయాలు",
  menuRowHistoryTitle: "చరిత్ర",
  menuRowHistoryDescription: "మా దేవాలయం గురించి",
  menuRowSevasTitle: "సేవలు",
  menuRowSevasDescription: "పూజలు మరియు సేవలు",
  menuRowFaqTitle: "ప్రశ్నలు",
  menuRowFaqDescription: "సాధారణ ప్రశ్నలు",
  menuRowDonationInfoTitle: "విరాళం",
  menuRowDonationInfoDescription: "ఎలా విరాళం ఇవ్వాలి",
  menuRowChangeLanguageTitle: "భాష",
  menuRowChangeLanguageDescription: "ఇంగ్లీష్ / తెలుగు మార్చండి",

  eventsEmpty: "ప్రస్తుతం రాబోయే కార్యక్రమాలు లేవు. దయచేసి తర్వాత మళ్లీ చూడండి.",
  eventsHeader: "{temple} దేవాలయంలో రాబోయే కార్యక్రమాలు:",
  eventsFooter: 'తిరిగి వెళ్ళడానికి "మెను" అని టైప్ చేయండి.',

  contactFallbackPhone: "దేవాలయ కార్యాలయం",
  contactEmailLabel: "ఇమెయిల్",
  contactDirectionsLabel: "దిశలు",

  unknownMessage: 'క్షమించండి, నాకు అర్థం కాలేదు. ఎంపికల కోసం "మెను" అని రిప్లై ఇవ్వండి.',

  announcementIntro: "నమస్తే. {temple} దేవాలయంలో రాబోయే కార్యక్రమం: {title}, {date} న {time}కి.",
  announcementFooter: 'రాబోయే కార్యక్రమాలు చూడటానికి "కార్యక్రమాలు" అని టైప్ చేయండి.',

  timingsClosedForOccasion: "{occasion} సందర్భంగా {temple} నేడు మూసివేయబడింది.",
  timingsNotConfigured: "దేవాలయ సమయాలు ఇంకా నమోదు చేయలేదు. దయచేసి దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",
  timingsHeader: "{temple} సమయాలు:",
  timingsHeaderWithOccasion: "{temple} సమయాలు ({occasion}):",
  timingsMorningLabel: "ఉదయం:",
  timingsEveningLabel: "సాయంత్రం:",

  historyFallback: "దేవాలయ చరిత్ర ఇంకా జోడించలేదు. దయచేసి దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",

  sevasEmpty: "ఇంకా సేవలు జాబితా చేయలేదు. సేవా వివరాల కోసం దయచేసి దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",
  sevasHeader: "{temple} దేవాలయంలో సేవలు:",
  sevasAvailableLabel: "అందుబాటులో",
  sevasTrailer: "\n\n...మరియు మరిన్ని. పూర్తి జాబితా కోసం దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",

  faqEmpty: "ఇంకా తరచుగా అడిగే ప్రశ్నలు జోడించలేదు. దయచేసి దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",
  faqHeader: "తరచుగా అడిగే ప్రశ్నలు:",
  faqTrailer: "\n\nమరిన్ని ప్రశ్నలు ఉన్నాయా? దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",

  donationInfoFallback: "విరాళాల సమాచారం ఇంకా జోడించలేదు. దయచేసి దేవాలయ కార్యాలయాన్ని సంప్రదించండి.",
  helpBody:
    "నమస్తే! ఈ చాట్‌బాట్‌ను ఎలా ఉపయోగించాలో ఇక్కడ ఉంది:\n\n" +
    'సంఖ్యతో రిప్లై ఇవ్వండి, లేదా "కార్యక్రమాలు" లేదా "సమయాలు" వంటి పదాన్ని టైప్ చేయండి.\n' +
    'అన్ని ఎంపికలను చూడటానికి ఎప్పుడైనా "మెను" అని టైప్ చేయండి.\n' +
    'ఇంగ్లీష్ మరియు తెలుగు మధ్య మారడానికి "భాష" అని టైప్ చేయండి.\n\n' +
    "దేవాలయ వాలంటీర్ కూడా సహాయం చేయడానికి సంతోషిస్తారు — మెనులోని సంప్రదించండి ఎంపికను చూడండి.",

  dayMonday: "సోమవారం",
  dayTuesday: "మంగళవారం",
  dayWednesday: "బుధవారం",
  dayThursday: "గురువారం",
  dayFriday: "శుక్రవారం",
  daySaturday: "శనివారం",
  daySunday: "ఆదివారం",

  languagePickerBody: "🙏 Welcome! Please choose your preferred language.\nదయచేసి మీ భాషను ఎంచుకోండి.",
  languagePickerButtonEnglish: "English",
  languagePickerButtonTelugu: "తెలుగు",
};
