...
/**
 * Statuses that permanently take a campaign off its public page.
 *
 * `completed` is NOT one of them: in this schema `status` tracks the WhatsApp
 * *broadcast* lifecycle, not the fundraiser (see lib/campaigns/run-campaign.ts
 * — a one-time donation campaign is deliberately left `running` after its
 * announcement sends). A campaign whose announcement finished going out is
 * routinely still collecting donations for weeks; whether the fundraiser is
 * over is answered by `campaignEndDate` alone. Same for `draft`/`scheduled`/
 * `paused` — the donation link must work for preview and manual sharing the
 * moment the campaign exists, never gated on a "Send Now" click.
 */
const TERMINAL_STATUSES: ReadonlySet<Campaign["status"]> = new Set(["archived", "cancelled"]);
