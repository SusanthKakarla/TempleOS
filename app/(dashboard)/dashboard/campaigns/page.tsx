...
export default async function CampaignsPage({ searchParams }: CampaignsPageProps) {
  ...
  const filter: ListCampaignsFilter = { search, status, campaignType, page, pageSize: DEFAULT_PAGE_SIZE };
  const [campaigns, totalCount] = await Promise.all([
    listCampaigns(session.tenantId, filter),
    countCampaignsFiltered(session.tenantId, { search, status, campaignType }),
  ]);
