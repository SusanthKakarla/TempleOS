...
export async function POST(req: NextRequest) {
  ...
  // Every row is validated independently — a shared phone number (within
  // this file or against an existing devotee) is never treated as a
  // duplicate; each row always produces its own devotee (see
  // lib/validation/devotee-import.ts).
  const validatedRows: PreviewRow[] = rawRows.map(({ rowNumber, raw }) => validateImportRow(rowNumber, raw));
